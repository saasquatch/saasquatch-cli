...
exports.default = function (program) {
  ...
  publish.description('Publish a theme').option('-t, --tenant [tenant]', 'required - which tenant to use').option('-k, --apiKey [apiKey]', 'required - which API key to use (for corresponding tenant)').action(function (options) {
    ...
    console.log('Publishing theme for %s with key %s', _chalk2.default.blue(tenant), _chalk2.default.blue(apiKey));
      ...
      getStatus(function (logItem) {
        if (logItem.timestamp > sinceTime) {
          ...
          if (logItem.message === magicConstants.completedMsg) {
            console.log(_chalk2.default.green(logItem.message));
          } else {
